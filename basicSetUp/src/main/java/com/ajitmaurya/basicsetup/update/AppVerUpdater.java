...
import com.ajitmaurya.basicsetup.update.callback.Callback;
...
public class AppVerUpdater extends DialogFragment {
        ...
        if (networkReceiver != null) {
            try {
                ...
                if (BuildConfig.DEBUG) {
                    BasicFunctions.log(TAG, "Exception: "+e.getLocalizedMessage());
                }
            }
        }
    }
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        if (context == null) {
            BasicFunctions.log(TAG, "App updater's context is not available anymore, cannot proceed");
            return null;
        }
        ...
        if (LibraryUtils.isUpdateAvailable(LibraryUtils.appVersion(context), response.getVersion())) {
            ...
            if (BuildConfig.DEBUG) {
                BasicFunctions.log(TAG, "UpdateInfo...");
            }
    ...
    private void failureCallback(Context context, final UpdateErrors error) {
        ((Activity) context).runOnUiThread(() -> callback.onFailure(error));
    }
}
